...
import { useCreateJustifMutation } from "../../store/api/MainApi";
import toast from "react-hot-toast";

const Justify = () => {
  const [file, setFile] = useState<File | null>(null);
  const [message, setMessage] = useState<string>("");
  const [justify,{isSuccess}] = useCreateJustifMutation()
        ...
        <div className="grid sm:grid-cols-2 items-start gap-14 p-8 mx-auto max-w-4xl bg-white shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-md font-[sans-serif]">
          <div>
            <h1 className="text-gray-800 text-3xl font-extrabold">
              Let's justify your hours
            </h1>
            <p className="text-sm text-gray-500 mt-4">
              provide any supporting documents or evidence regarding your recent
              absence. This will help us understand the situation better.
            </p>
          ...
          <form className="ml-auo space-y-4" onSubmit={(e)=>{
            e.preventDefault()
            handleSubmit()
          }} >
            <input
              ...
              placeholder="Name"
              onChange={(e) => {
                const selectedFile = e.target.files?.[0]; // Get the first
